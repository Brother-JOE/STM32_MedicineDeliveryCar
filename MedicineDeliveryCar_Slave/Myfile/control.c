...
uint8_t slave_mid_start_flag = 0;
uint8_t slave_get_start_flag = 0;
uint8_t slave_restart2far_flag = 0;
...
void Spin_Turn(SpinDir_t goal_dir)
{
    ...
    if(goal_dir == left_90)
    {
        ...
        pid_target_set(&pid_distance_l, -0.25*3.1416*180);
        pid_target_set(&pid_distance_r, 0.25*3.1416*180);
    }
