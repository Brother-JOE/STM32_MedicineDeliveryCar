...
uint8_t U4RxBuffer[RXBUFFERSIZE] = {0};
uint8_t U4RXdata_len = 0;
uint8_t u4_aRxBuffer;			
uint8_t Uart4_Rx_Cnt = 0;		
uint8_t uart4_rxstate = 0;
uint8_t uart4_rx_finish = 0;
uint8_t uart4_tx_finish = 0;
//todo
/* USER CODE END 0 */
  ...
  void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  if(huart->Instance == UART4)//*UART4
    {

      if((u4_aRxBuffer == 0xD1)&&(uart4_rxstate == 0))
      {
        U4RxBuffer[Uart4_Rx_Cnt++] = 0xD1;
        uart4_rxstate = 1;
      }
      else if(uart4_rxstate == 1)
      {
        U4RxBuffer[Uart4_Rx_Cnt++] = u4_aRxBuffer;
        if((u4_aRxBuffer == 0xD2)||(Uart4_Rx_Cnt >= RXBUFFERSIZE))
        {
          uart4_rxstate = 2;
        }
      ...
      if(uart4_rxstate == 2)
      {
        if((U4RxBuffer[0] == 0xD1)&&(U4RxBuffer[Uart4_Rx_Cnt-1]== 0xD2))
        {
          /*Reception Complete*/
        ...
        else
        {
          uart4_rxstate = 0;
          Uart4_Rx_Cnt = 0;
          memset(U4RxBuffer,0x00,sizeof(U4RxBuffer));
        }
