...
void MX_FREERTOS_Init(void) {
  ...
  osThreadDef(CarGoalCtrlTask, Task_CarGoalCtrl, osPriorityBelowNormal, 0, 512);
  ...
  for(;;)
  {
    ...
    if(line_timer == 2){
      ...
      if(dis_ctrl_flag == 1)
      {
        ...
        dis_outval_r = right_distance_control();
        pid_target_set(&l_pid, dis_outval_l);   //set speed
        pid_target_set(&r_pid, dis_outval_l);   //set speed //?ttttttt

        if(errfin_cnt_l_dis < POINT_FIT_TIMES){ //?|| errfin_cnt_r_dis < POINT_FIT_TIMES){
            if(fabs(pid_distance_l.err) < ERR_FIT_VALUE) errfin_cnt_l_dis++;
            else errfin_cnt_l_dis /=2;
            // if(fabs(pid_distance_r.err) < ERR_FIT_VALUE) errfin_cnt_r_dis++; //?test
            // else errfin_cnt_r_dis /=2;
        }
        else{
          dis_ctrl_fin_flag = 1;
  ...
  for(;;)
  {
    ...
    if(delivery_flag == 0)
    {
      if(TASK == 0)
      {
        ...
        {
          if( (finded_flag == 1) && (Drug_ReadState() == GPIO_PIN_RESET) )  //****
          {
            //LED_YELLOW_OFF();         //?test

            finded_flag = 0;
            speed_target = 140.0;   //0.5m/s  //todo speed
            line_flag = 1;
            cur_action++;        
          }
        }
      ...
      else if(TASK == 2)  //area 'E'~'H'  
      {
        if(cur_action == 0)
        {
          if(crossing_flag == 1 && (int)(Left_getpos() - left_last_dis_cnt)*213.63/4/390 > 200) //
          {
            crossing_flag = 0;    
            line_flag = 0;
            //Car_go(100);  //100mm //?若要识别four——num则取消注释
            //cur_action++;
            Car_go(200);    //200mm //?
            cur_action = 5;
            LoR = 1;
        ...
        else if(cur_action == 2)
        {
          if(spin_fin_flag == 1)
          {
            ...
            pid_target_set(&l_pid, 0);
            pid_target_set(&r_pid, 0);            
            //HAL_UART_Transmit_IT(&huart1, get_four_dir_cmd, sizeof(get_four_dir_cmd));  //!test
            cur_action++;
            //!test
            finded_flag = 1;
            LoR = -1;
        ...
        else if(cur_action == 6)
        {
          if(spin_fin_flag == 1)
          {
            ...
            pid_target_set(&r_pid, speed_target);
            line_flag = 1;
            left_last_dis_cnt = Left_getpos();  //防干扰
            crossing_flag = 0;
            cur_action++;
          }  
        }
        ...
        {
          if(crossing_flag == 1 && (int)(Left_getpos() - left_last_dis_cnt)*213.63/4/390 > 230) 
          {
            crossing_flag = 0;    
            line_flag = 0;
            Car_go(-35);  //-35mm
        ...
        {
          if(crossing_flag == 1 && (int)(Left_getpos() - left_last_dis_cnt)*213.63/4/390 > 1200) //
          {
            crossing_flag = 0;    
            line_flag = 0;
            Car_go(-30);  //-30mm
    ...
    else if(delivery_flag == 1) //Go Back
    {
      if(TASK == 0)
      {
        if(cur_action == 0)
        {
          if(Drug_ReadState() == GPIO_PIN_SET)  //药品被拿走
          {
            LED_RED_OFF();
            Spin_Turn(back_180);
            cur_action++;

            if(fin_area == 'C' || fin_area == 'D'){
              HAL_UART_Transmit_IT(&huart2, make_slave_led_off, sizeof(make_slave_led_off)); //?for slave
            }
            else if(fin_area >= 'E'){
              //?for slave
              HAL_UART_Transmit_IT(&huart2, make_slave_getdrugstart_cmd, sizeof(make_slave_getdrugstart_cmd)); 
            }
          }
        }
        else if(cur_action == 1)
        ...
        else if(cur_action == 4)
        {
          if(spin_fin_flag == 1)
          {
            ...
            if(fin_area >= 'A' && fin_area <= 'D'){   //直接巡线到终点
              ...
              if(fin_area == 'C' || fin_area == 'D'){
                HAL_UART_Transmit_IT(&huart2, make_slave_start_cmd, sizeof(make_slave_start_cmd)); //?for slave
              }
      ...
      else if(TASK == 1)  //巡线到终点
      {
        if(cur_action == 0)
        {
          if(stop_flag == 1)
          {
            ...
            if(fin_area >= 'E'){
              //?for slave
              HAL_UART_Transmit_IT(&huart2, make_slave_restart2far_cmd, sizeof(make_slave_restart2far_cmd)); 
            }
          }
        }
        else if(cur_action == 1)
